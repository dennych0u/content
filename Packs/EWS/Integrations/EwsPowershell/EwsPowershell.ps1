...
class OAuth2Client {
    ...
    [PSObject]AuthorizationRequest() {
        ...
        $response = Invoke-RestMethod 'https://login.microsoftonline.com/organizations/oauth2/v2.0/devicecode' -Method 'POST' -Headers $headers -Body $body
        $this.device_code = $response.device_code
        $this.device_code_creation_time = [int][double]::Parse((Get-Date -UFormat %s))
        $this.device_code_expires_in = $response.expires_in
    ...
    [PSObject]AccessTokenRequest() {
        try {
            ...
            $response = Invoke-WebRequest 'https://login.microsoftonline.com/organizations/oauth2/v2.0/token' -Method 'POST' -Headers $headers -Body $body
        ...
        $this.access_token_expires_in = $response_body.access_token_expires_in
...
2. Run the following command **!ews-complete-auth** in the Waroom."
    $entry_context = @{}
...
function RefreshAuthCommand ([Oauth2Client]$client) {
    ...
    $human_readable = "**Token refreshed**"
    $entry_context = @{}
...
function Main {
	...
	try {
        ...
        switch ($command) {
            ...
            "$global:COMMAND_PREFIX-refresh-token" {
